...
# Set this variable to True to repeat the sound on each bounce,
# or False to play the sound continuously
repeat_sound_on_bounce = False  # Set to False for continuous playback
...
pygame.mixer.init()
pygame.mixer.music.load('song.ogg')  # Use the OGG file
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        elif repeat_sound_on_bounce and event.type == MUSIC_PAUSE_EVENT:
...
pygame.quit()  # Quit Pygame before using MoviePy

# Use MoviePy to write the video
from moviepy.editor import ImageSequenceClip

# Convert frames to the correct format (MoviePy uses (height, width, channels))
