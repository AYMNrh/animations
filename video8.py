...
videos_to_generate = [
    {
        ...
        'video_duration': 65,  # Desired video duration in seconds
    },
    {  
        'song_file': 'crazyfrog.mp3',
        'video_duration': 65,  # Desired video duration in seconds
    },
    {
        'song_file': 'rushe.mp3',
...
def draw_radial_gradient(surface, center, radius, inner_color, outer_color):
    for i in range(int(radius), 0, -1):
        ...
        color = (r, g, b)
        pygame.gfxdraw.filled_circle(surface, center[0], center[1], i, color)
...
for idx, video_settings in enumerate(videos_to_generate):
    ...
    video_filename = f"{base_name}_no_audioc.mp4"  # Intermediate video without audio
    final_video_filename = f"{base_name}_with_audioc.mp4"  # Final video with synchronized audio
    ...
    background_bottom_color = (0, 0, 0)  # Marine Blue  0 0 128
    ...
    for i in range(num_balls):
        ...
        ball = {
            ...
            'pulses': [],        # Pulses
            'trail': [],         # Trail effect
            'trail_length': 15,  # Length of the trail
            'initial_radius': 20,
            'expansion_rate': 1.05,
        }
        balls.append(ball)
    ...
    while running:
        ...
        # Draw the boundary circle with a thicker and smoother line
        pygame.gfxdraw.aacircle(screen, boundary_center[0], boundary_center[1], boundary_radius, boundary_color)
        pygame.gfxdraw.aacircle(screen, boundary_center[0], boundary_center[1], boundary_radius - 1, boundary_color)
        pygame.gfxdraw.aacircle(screen, boundary_center[0], boundary_center[1], boundary_radius - 2, boundary_color)
        # pygame.gfxdraw.aacircle(screen, boundary_center[0], boundary_center[1], boundary_radius - 3, boundary_color)
        ...
        for ball in balls[:]:  # Use a copy of the list in case we need to modify it
            ...
            if ball['bounce_times'] and t >= ball['bounce_times'][0]:
                ...
                ball['target_color'] = random_color()
                ball['color_transition_progress'] = 0.0
            ...
            if ball['color_transition_progress'] < 1.0:
                ball['color_transition_progress'] += ball['color_transition_speed']
                if ball['color_transition_progress'] > 1.0:
                    ball['color_transition_progress'] = 1.0

                # Interpolate colors
                r = lerp(ball['color'].r, ball['target_color'].r, ball['color_transition_progress'])
                g = lerp(ball['color'].g, ball['target_color'].g, ball['color_transition_progress'])
                b = lerp(ball['color'].b, ball['target_color'].b, ball['color_transition_progress'])
                ball['color'] = Color(int(r), int(g), int(b))
            else:
                ball['color'] = ball['target_color']
            ...
            for pulse in ball['pulses'][:]:
                ...
                if pulse['alpha'] <= 0 or pulse['radius'] >= pulse['max_radius']:
                    ball['pulses'].remove(pulse)
                    continue
                # Draw the pulse
                pulse_surface = pygame.Surface((int(pulse['radius'] * 2), int(pulse['radius'] * 2)), pygame.SRCALPHA)
                pulse_alpha = int(pulse['alpha'])
                pulse_color = (pulse['color'].r, pulse['color'].g, pulse['color'].b, pulse_alpha)
                pygame.gfxdraw.filled_circle(pulse_surface, int(pulse['radius']), int(pulse['radius']), int(pulse['radius']), pulse_color)
                screen.blit(pulse_surface, (pulse['x'] - pulse['radius'], pulse['y'] - pulse['radius']))

            # Update trail
            ball['trail'].append({'x': ball['x'], 'y': ball['y'], 'color': ball['color']})
            if len(ball['trail']) > ball['trail_length']:
                ball['trail'].pop(0)
            # Draw trail
            for i, trail_point in enumerate(ball['trail']):
                trail_alpha = int(255 * (i + 1) / ball['trail_length'])
                trail_surface = pygame.Surface((int(ball['radius'] * 2), int(ball['radius'] * 2)), pygame.SRCALPHA)
                trail_color = (trail_point['color'].r, trail_point['color'].g, trail_point['color'].b, trail_alpha)
                pygame.gfxdraw.filled_circle(trail_surface, int(ball['radius']), int(ball['radius']), int(ball['radius']), trail_color)
                screen.blit(trail_surface, (trail_point['x'] - ball['radius'], trail_point['y'] - ball['radius']))

            # Draw the ball with radial gradient
            circle_surface = pygame.Surface((int(ball['radius'] * 2), int(ball['radius'] * 2)), pygame.SRCALPHA)
