...
# Segment duration for audio segments
...
for idx, video_settings in enumerate(videos_to_generate):
    ...
    # Initialize current position in the song
    current_pos = 0.0
    ...
    while running:
        ...
        if distance_from_center + circle_radius >= boundary_radius:
            ...
            bounce_times.append({'time': t, 'current_pos': current_pos})

            # Increment current_pos by segment_duration
    ...
    if repeat_sound_on_bounce:
        ...
        # Use segments of the song file at each bounce

        # Load the song file
        song_clip = AudioFileClip(continuous_audio_filename)

        # Create a list of song segments at the recorded times
        audio_segments = []
        for bounce in bounce_times:
            time = bounce['time']
            start_in_song = bounce['current_pos'] - segment_duration  # Adjusted to play from current_pos
            if start_in_song < 0:
                start_in_song = 0  # Ensure we don't have negative start time

            # Ensure we don't exceed the song's duration
            if start_in_song + segment_duration <= song_clip.duration:
                segment = song_clip.subclip(start_in_song, start_in_song + segment_duration).set_start(time)
                audio_segments.append(segment)
            else:
                # If the segment goes beyond the song's duration, adjust the duration
                adjusted_duration = song_clip.duration - start_in_song
                if adjusted_duration > 0:
                    segment = song_clip.subclip(start_in_song, song_clip.duration).set_start(time)
                    audio_segments.append(segment)
                else:
                    break  # No more audio to play

        # Combine the audio segments, mixing overlapping parts
        if audio_segments:
            final_audio = CompositeAudioClip(audio_segments).set_duration(video_duration)
        else:
            final_audio = None  # No bounces occurred; no audio
